...
class Weather2Processor(data_processor):
    """
        this is description of Weather processor
    """
    __type__ = 'Weather2Processor'
    ...
    def read(self,filename):
        try:
            ...
            try:
                ...
                if dict_data['stationId'] == "C0A9C0":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5372,25.1175]}
                if dict_data['stationId'] == "C0A9E0":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5030,25.0903]}
                if dict_data['stationId'] == "C0A9F0":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5755,25.0794]}


                if dict_data['stationId'] == "C0AC40":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5224,25.1757]}
                if dict_data['stationId'] == "C0AC70":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5757,25.0024]}
                if dict_data['stationId'] == "C0AC80":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5646,25.0378]}

                if dict_data['stationId'] == "C0AH40":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5771,25.1291]}
                if dict_data['stationId'] == "C0AH70":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5504,25.0487]}
                if dict_data['stationId'] == "C0AI40":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5132,25.1156]}
                if dict_data['stationId'] == "C1A730":
                    dict_data['location'] = {'type':'Point','coordinates':[121.5395,25.0143]}
                if dict_data['stationId'] == "C1AC50":
                    dict_data['location'] = {'type':'Point','coordinates':[121.4693,25.1334]}
            except:
