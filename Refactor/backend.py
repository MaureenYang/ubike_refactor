...
def getStationInformation():  # can be rewrite by use Youbike processor
    ...
    json_file = open("raw_data/YouBikeTPNow.json")
...
model = model2 = pickle.load(open('GB_stat0.pkl','rb'))
...
def getBikeDatabyDate(sno,datePicked):
    ...
    try:
        ...
        true_bemp_df = selected_df[piked_dt_idx].astype(int)
        true_sbi_df = selected_df[piked_dt_idx].astype(int)

        true_bemp_df = true_bemp_df[['predict_hour','y_bemp']]
        true_sbi_df = true_sbi_df[['predict_hour','y_sbi']]

        pre_empty_df = selected_df[piked_dt_idx][['predict_hour','y_bemp_predict']]
        pre_bike_df = selected_df[piked_dt_idx][['predict_hour','y_sbi_predict']]
...
if __name__ == '__main__':
    ...
    a = getBikeDatabyDate(1,dt.datetime.strptime("2018-03-02 01:00:00", "%Y-%m-%d %H:%M:%S"))
