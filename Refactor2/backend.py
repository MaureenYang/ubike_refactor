...
class weather_station(object):
    ...
    def __init__(self, wdict):
        ...
        if self.weather_id in ['466910','466920']:
...
class bike_station(object):
    ...
    def __init__(self, bdict):
        ...
        self.cur_dt = datetime.datetime.strptime(bdict['cur_dt'], "%Y%m%d%H%M%S")
...
class backend(object):
    ...
    def set_wstation_by_id(self,station_id):
        min_dist = 9999
        min_station = None
    ...
    def update_all_station(self):
        ...
        if not self.all_station: #dict empty_df
            ...
            for key, value in ubike_data['retVal'].items():
                bs_dict = {}
                bs_dict['sno'] = value['sno']
                bs_dict['name'] = value['sna']
                bs_dict['addr'] = value['ar']
                bs_dict['lat'] = value['lat']
                bs_dict['lng'] = value['lng']
                bs_dict['tot'] = value['tot']
                bs_dict['sbi'] = value['sbi']
                bs_dict['bemp'] = value['bemp']
                bs_dict['cur_dt'] = value['mday']
                bs_dict['act'] = value['act']
                bs = bike_station(bs_dict)
                self.all_station[int(value['sno'])] = bs
            self.station_num = len(bs_dict)
            # find nearest weather station for each bike station
            for key in self.all_station:
                bs = self.all_station[key]
                self.set_wstation_by_id(bs.sno)
        else:
