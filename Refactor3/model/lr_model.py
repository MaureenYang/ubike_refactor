...
def LinearReg(X, Y, kfold=3, feature_set=None):
    ...
    lr_base.fit(train_X, train_y)
    print('Base Parameters in use:\n')
    print(lr_base.get_params())
    ...
    lr_grid = GridSearchCV(estimator=lr_base, param_grid=grid_grid, scoring='neg_mean_squared_error', cv = ps2.split(), verbose=2, n_jobs=-1)

    # Fit the grid search model
    lr_grid.fit(train_X, train_y)
    BestPara_grid = lr_grid.best_params_
    print("grid search, best parameter:", lr_grid.best_params_)
    cv_results_grid = lr_grid.cv_results_

    #prediction
    predict_y_grid = lr_grid.predict(test_X)
    predict_y_base = lr_base.predict(test_X)
